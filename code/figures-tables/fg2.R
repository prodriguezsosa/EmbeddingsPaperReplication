...
# these are the top 10 nearest neighbors for each of the 10 cues generated by the models we evaluated
nn_list <- readRDS("data/mturk/triad_task/input_data/nn_list.rds")
names(nn_list)[1] <- "pretrained" # rename glove pretrained to match tt output
...
## bootstrap relative performance statistic and plot
plot_list <- list('2a' = c("48_300", "6_300"), '2b' = c("6_300", "human"), '2c' = c("pretrained", "6_300"), '2d' = c("pretrained", "human"))

for(i in names(plot_list)){
  
  # select models to be compared
  models = plot_list[[i]] # models[2] is the baseline
  
  # compute probability of the same token being selected from both models
  ...
  ggsave(paste0('./figures/', 'fg', i, '.eps'), dpi = 800)
}
