...
fig_numbers <- c(14, 15, 16, 17)

## loop through all corpora
for(i in 1:length(corpora)){
  
## load data
within_corr_politics <- readRDS(paste0('./data/', corpora[i],'/glove/correlations/within_corr_politics.rds'))
within_corr_random <- readRDS(paste0('./data/', corpora[i],'/glove/correlations/within_corr_random.rds'))
...
ggplot(within_corr_dt, aes(x = dim, y = avg, color = window, position_dodge(width=0.5))) + 
  ...
  theme(plot.title = element_text(size=20, margin = margin(t = 20, r = 0, b = 0, l = 0)), panel.background = element_blank(),
        ...
        legend.position = "top")
ggsave(paste0('./figures/fg', fig_numbers[i],'a_app.eps'), dpi = 800)
...
ggplot(within_corr_dt, aes(x = dim, y = avg, color = window, position_dodge(width=0.5))) + 
  ...
  theme(plot.title = element_text(size=20, margin = margin(t = 20, r = 0, b = 0, l = 0)), panel.background = element_blank(),
        ...
        legend.key=element_blank(),
        legend.position = "top")
ggsave(paste0('./figures/fg', fig_numbers[i],'b_app.eps'), dpi = 800)
