...
corpora <- c('ps', 'sotu', 'sp', 'gr')
fig_numbers <- c(10, 11, 12, 13)

## loop through all corpora
for(i in 1:length(corpora)){
  
## list embedding models
embeddings_file <- as.list(list.files(paste0('./data/', corpora[i], '/glove/models/')))
...
extract_computation_data <- function(model){
  ...
  # load files
  cost_data <- paste0('./data/', corpora[i], '/glove/models/', cost_files) %>% map(readRDS) 
  time_data <- lapply(time_files, function(x) readRDS(paste0('./data/', corpora[i], '/glove/models/', x))) %>% lapply(., function(x) x[[1]] %>% as.numeric) %>% unlist
  
  # align units of time data
  time_data_units <- lapply(time_files, function(x) readRDS(paste0('./data/', corpora[i], '/glove/models/', x))) %>% lapply(., function(x) attributes(x[[1]])$units) %>% unlist
...
ggplot(computation_data, aes(x = as.numeric(as.character(dim)), y = mean_cost, group = window)) +
  ...
  theme(plot.title = element_text(size=20, margin = margin(t = 20, r = 0, b = 0, l = 0)), panel.background = element_blank(),
        ...
        legend.position = "top")
ggsave(paste0('./figures/fg', fig_numbers[i],'a_app.eps'), dpi = 1200)
...
ggplot(computation_data, aes(x = as.numeric(as.character(dim)), y = mean_time, group = window)) +
  ...
  theme(plot.title = element_text(size=20, margin = margin(t = 20, r = 0, b = 0, l = 0)), panel.background = element_blank(),
        ...
        legend.key=element_blank(),
        legend.position = "top")
ggsave(paste0('./figures/fg', fig_numbers[i],'b_app.eps'), dpi = 1200)
