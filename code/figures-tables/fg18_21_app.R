...
corpora <- c('ps', 'sotu', 'sp')
fig_numbers <- c(18, 19, 20)
...
## load data
corr_politics <- readRDS(paste0('./data/', corpora[i],'/glove/correlations/across_corr_pearson_politics.rds'))
corr_random <- readRDS(paste0('./data/', corpora[i],'/glove/correlations/across_corr_pearson_random.rds'))
...
ggplot(data = melted_adj, aes(Var2, Var1, fill = value)) +
  ...
  scale_fill_gradient2(low = "gray100", high = "gray40", mid = 0.65, space = "Lab", name="Pearson\nCorrelation") + # 0.2 is around min for random set
  theme_minimal() + 
...
ggsave(paste0('./figures/fg', fig_numbers[i],'a_app.png'))
...
ggplot(data = melted_adj, aes(Var2, Var1, fill = value)) +
  ...
  theme(
    ...
    axis.text.y = element_text(size=18),
    panel.grid.major = element_blank())
ggsave(paste0('./figures/fg', fig_numbers[i],'b_app.png'))
...
#------------------------------------------------------------
# heatmap for German corpus
# note: there were no readily available pre-trained GloVe embeddings 
#------------------------------------------------------------

## load data
corr_politics <- readRDS('./data/gr/glove/correlations/across_corr_pearson_politics.rds')
corr_random <- readRDS('./data/gr/glove/correlations/across_corr_pearson_random.rds')

## custom function to generate heatmap data
generate_heatmap_data <- function(corr_data){
  
  ## get model names
  models <- unique(c(corr_data$model1, corr_data$model2))
  models <- models[!grepl(paste("12", "24", "100", "200", sep = "|"), models)] # reduce number of models for visualization purposes
  ...
  # order
  adjorder <- c("1_50", "1_300", "1_450", "6_50", "6_300", "6_450", "48_50", "48_300", "48_450")
  adj <- adj[adjorder, adjorder]
...
ggplot(data = melted_adj, aes(Var2, Var1, fill = value)) +
  ...
  theme(
    ...
    axis.text.y = element_text(size=18),
    panel.grid.major = element_blank())
ggsave('./figures/fg21a_app.png')

## heatmap for politics cues
melted_adj <- generate_heatmap_data(corr_politics)
ggplot(data = melted_adj, aes(Var2, Var1, fill = value)) +
  geom_tile(color = "white") +
  scale_fill_gradient2(low = "gray100", high = "gray40", mid = 0.65, space = "Lab", name="Pearson\nCorrelation") + # 0.2 is around min for random set
